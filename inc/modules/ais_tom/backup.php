...
class backup_restore_theme_options {
                ...
                if ($options) {
                    foreach ($options as $option) {
                        ...
                        if ( unserialize($value) == false ) {
                            ...
                            update_option($option->option_name, unserialize( $value ) );
                        }
                    }
                }
            }
    ...
    function _get_options() {
        global $wpdb;
        return $wpdb->get_results("SELECT option_name, option_value FROM {$wpdb->options} WHERE option_name LIKE '%ais_contact_%' OR option_name LIKE '%ais_flightstats_%'"); // edit 'shapeSpace_options' to match theme options
    }
}
